...
class Lecture():
    ...
    def get_current_tmux_session(self):
        try:
            # Run the tmux command to get the current session name
            session_name = subprocess.check_output(["tmux", "display-message", "-p", "#S"]).decode("utf-8").strip()
            return session_name
    ...
    def edit(self):
        # Get the path to the lecture file (replace with your actual file path)
        lecture_file_path = str(self.file_path)

        # Get the current tmux session name
        tmux_session = self.get_current_tmux_session()

        if tmux_session:
            # Use '--' to separate the command from tmux arguments
            command = f"tmux send-keys -t {tmux_session} 'nvim {lecture_file_path}' C-m"
            subprocess.Popen(["sh", "-c", command])
        else:
            print("Unable to determine the current tmux session.")
...
class Lectures(list):
    ...
    def compile_master(self):
        result = subprocess.run(
            ['latexrun', '--latex-args="-interaction=nonstopmode"', str(self.master_file)],
